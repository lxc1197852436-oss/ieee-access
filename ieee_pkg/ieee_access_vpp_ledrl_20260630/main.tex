...
\begin{abstract}
Virtual power plants (VPPs) increasingly operate under renewable uncertainty, market-price volatility, and external operational events such as weather warnings, demand-response notices, and renewable-curtailment alerts. Most reinforcement-learning-based dispatch methods consume only numerical states and therefore cannot directly exploit such natural-language information. To address this limitation, this paper develops a language-enhanced deep reinforcement learning framework for event-aware VPP battery dispatch. The proposed LE-DRL-SAC architecture employs a large language model (DeepSeek) as a semantic encoder to transform operational text into structured risk features—namely, general risk, price-spike pressure, load pressure, renewable-curtailment pressure, and a storage-bias signal. These features are fused with numerical states and consumed by a Soft Actor-Critic (SAC) policy; the language model does not directly emit dispatch actions. To enhance operational reliability, the learned SAC action is further combined with a deterministic semantic safety layer that encodes risk-consistent charging and discharging preferences. The framework is evaluated on a public-data-calibrated 15-minute VPP dispatch dataset comprising load, photovoltaic generation, electricity price, temperature, battery state of charge, and Chinese operational event text. Four scenarios are examined: normal summer operation, high-temperature load pressure, price spike, and renewable-curtailment pressure. Across three random seeds, the proposed LE-DRL-SAC with semantic safety layer ($w=0.9$) attains an average total reward of -208,468.9 yuan, outperforming a rule-based controller (-208,969.9 yuan), an engineering-regularized enhanced rolling-horizon optimizer (-209,988.3 yuan), numerical SAC (-211,023.2 yuan), a text-ablation SAC (-211,023.1 yuan), and a diagnostic SAC-Numeric with a purely numeric safety layer (-209,125.3 yuan). Bootstrap resampling over three seeds confirms that the improvement over numerical SAC, the text-ablation SAC, and the pure learned actor is statistically significant (95\% confidence intervals exclude zero). The numeric safety layer ablation demonstrates that the gain is not driven solely by the safety-layer mechanism, because a text-agnostic numeric safety layer improves numerical SAC yet remains below the language-enhanced controller. Text ablation and prior-weight sweeps further corroborate that the improvement stems from operational text semantics coupled with the safety-layer design rather than from input-dimensionality expansion alone. The findings indicate that textual risk semantics can improve event-driven VPP dispatch when coupled with a reproducible action-safety mechanism. An out-of-distribution evaluation on real Guangzhou weather observations further shows that the proposed controller retains the highest average reward when the temperature and PV channels are drawn from a distribution distinct from the synthetic training data, whereas validation on measured VPP telemetry and full MILP/MPC baselines remains necessary.
\end{abstract}
...
\begin{figure*}[t!]
  ...
  \caption{Language-enhanced VPP dispatch framework with an auditable semantic safety layer. (a) Event and numerical inputs from a public-data-calibrated 15-minute trajectory. (b) DeepSeek maps operational text to a cached risk vector and never emits a dispatch action. (c) The augmented state drives the SAC actor, and the semantic safety layer blends the actor action with the semantic prior as $a=(1-w)a^{\mathrm{SAC}}+w a^{\mathrm{sem}}$ ($w=0.9$, prior power $2.0$ MW). (d) The final action is clipped by battery feasibility and evaluated by reward, CVaR, throughput, and an out-of-distribution real-weather test.}
...
Table~\ref{tab:scenario_results} reports the core scenario-level comparison. The proposed controller obtains the best total reward in S1, S2, and S4. In S3, Rolling-Horizon remains stronger because short-term look-ahead is especially effective under explicit price-spike dynamics. The SAC-Numeric + numeric safety layer variant is a strong scenario-level baseline, confirming that a numeric safety layer already provides event-responsive behavior; however, the language-enhanced controller still exceeds it on the cross-scenario average. The Enhanced Rolling-Horizon optimizer achieves low battery throughput in every scenario, illustrating the engineering-regularized trade-off between cycling cost and economic reward.
...
\midrule
\multirow{8}{*}{S1} & LE-DRL-SAC + safety layer & \textbf{-201,654.0} & \textbf{-644.5} & 25.30 & 0.222 \\
...
\midrule
\multirow{8}{*}{S2} & LE-DRL-SAC + safety layer & \textbf{-218,759.4} & \textbf{-722.1} & 23.17 & 0.196 \\
...
\midrule
\multirow{8}{*}{S3} & Rolling-Horizon & \textbf{-218,749.2} & -880.2 & 131.17 & 0.458 \\
& LE-DRL-SAC + safety layer & -218,660.5 & \textbf{-797.4} & 25.38 & 0.183 \\
...
\midrule
\multirow{8}{*}{S4} & LE-DRL-SAC + safety layer & \textbf{-194,801.9} & \textbf{-622.6} & 39.61 & 0.323 \\
...
\begin{figure}[t!]
  ...
  \caption{OOD cross-scenario average total reward on real Guangzhou weather observations (July 2024) with scenario variability. The proposed controller retains the highest average reward when the temperature and PV channels come from a distribution distinct from the synthetic training data.}
  \label{fig:ood_reward}
\end{figure}

\subsection{Generalization to Unseen Event Expressions}
\label{sec:unseen_text}
A natural concern is whether the DeepSeek semantic encoder merely matches the five fixed training templates rather than understanding event semantics. To examine this, an unseen-expression test set was constructed in which each event category was rewritten into five paraphrases that differ in wording from the training template but preserve the operational meaning (for example, the high-temperature warning was paraphrased as ``南方区域持续高温，空调负荷预计在晚高峰快速攀升'' and ``气象部门提示未来数小时出现极端炎热天气''). DeepSeek was queried for each unseen expression, and the resulting scores were consistent with the training-template scores at the category level: high-temperature and demand-response expressions received high load-pressure scores ($\approx 0.84$--$0.86$), price-spike expressions received high price-spike scores ($\approx 0.86$), and renewable-curtailment expressions received high curtailment scores ($\approx 0.80$), with storage-bias directions correctly negative for discharge-oriented events and positive for curtailment events. This indicates that the encoder generalizes beyond rote template matching.
...
This paper has presented LE-DRL-SAC with a semantic safety layer for event-aware VPP battery dispatch. The proposed framework employs an LLM-compatible semantic encoder to transform operational text into structured risk features, fuses these features with numerical states, trains an SAC actor under semantic-consistency guidance, and combines the learned action with a bounded semantic prior at test time. On a public-data-calibrated 15-minute VPP dispatch dataset, the final controller with $w=0.9$ outperforms numerical SAC, a text-ablation SAC, a numeric-safety-layer SAC variant, Rule-Based control, the Rolling-Horizon optimizer, and the engineering-regularized Enhanced Rolling-Horizon optimizer in average total reward across four event scenarios, with the improvement over all learned baselines confirmed as statistically significant by bootstrap confidence intervals. An out-of-distribution evaluation on real Guangzhou weather observations (July 2024) further demonstrates that the proposed controller retains the highest average reward when the temperature and PV channels are drawn from a distribution distinct from the synthetic training data, with the advantage concentrated in event-driven scenarios, providing evidence of generalization beyond the training set. A second generalization test on unseen event expressions confirms that the DeepSeek semantic encoder and the resulting policy are driven by semantic content rather than by memorized templates, as the controller retains the highest reward under paraphrased event wording. A numeric safety layer ablation demonstrates that the gain is not solely attributable to the blending mechanism, because a text-agnostic numeric safety layer improves SAC-Numeric yet remains below the language-enhanced controller. Collectively, the findings indicate that language-derived risk semantics are most effective when coupled with a deterministic action-safety mechanism rather than treated as passive state features. Future work will validate the framework on measured VPP event logs, incorporate full MILP/MPC baselines, and introduce formal safety-constrained RL to enhance deployability.
