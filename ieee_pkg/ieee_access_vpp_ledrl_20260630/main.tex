...
\begin{enumerate}
  ...
  \item An unseen-event adaptation study is reported. A fifth event category—negative-price deep surplus—that was not used during prior design or actor training is constructed, and the LLM semantic encoder is queried once for this unseen event to obtain cached risk scores. The study shows that when the semantic regularizer is relaxed, the language-enhanced policy exceeds numerical SAC on this unseen event (mean reward gap +5,127 yuan, 95\% bootstrap CI [+4,420, +5,655], all three seeds favorable), providing evidence that LLM-derived features help a reinforcement-learning dispatch policy adapt to an event category that the hand-crafted prior cannot cover.
...
\begin{figure*}[t!]
  ...
  \caption{Language-enhanced VPP dispatch framework with an auditable semantic safety layer. (a) Event and numerical inputs from the calibrated VPP dataset. (b) DeepSeek maps operational text to a cached risk vector and never emits a dispatch action; the LLM serves as an advisor, and the SAC actor is the learned decision component. (c) The augmented state drives the SAC actor, and the semantic safety layer blends the actor action with the semantic prior as $a=(1-w)a^{\mathrm{SAC}}+w a^{\mathrm{sem}}$ (prior power $2.0$ MW). The blending weight $w$ is event-coverage-dependent: high on event categories used to design the prior (known events, main configuration $w=0.9$) and low on unseen event categories, where the prior has no matching branch and the learned actor takes over (Section~\ref{sec:unseen_event}). (d) The final action is clipped by battery feasibility and evaluated by reward, CVaR, throughput, an out-of-distribution real-weather test, and an unseen-event adaptation test.}
...
\midrule
LE-DRL-SAC, relaxed regularizer, $w=0$ & \textbf{-141,155.6} & 1.00 \\
SAC-Numeric (retrained on S1--S5) & -146,282.4 & 0.98 \\
...
\textbf{Regularizer sensitivity.} Table~\ref{tab:actor_loss_sweep} sweeps the actor semantic-consistency regularizer weight on the S1--S5 training set. On the four known-event scenarios the reward is essentially flat across regularizer weights (within 0.5\%), confirming that the regularizer is not a load-bearing component on known events. On S5, however, the reward improves monotonically as the regularizer is relaxed: from $-159{,}670.6$ yuan at the full weight to $-141{,}155.6$ yuan at zero weight, a gain of 18{,}515.0 yuan. The known-event rewards do not degrade under this relaxation. This indicates that the regularizer plays two distinct roles that conflict on unseen events: it aligns the actor with the semantic target on known categories, but the same alignment prevents the actor from learning the cross-interval SOC management that the unseen event requires.
