...
\textbf{Result 4 --- DeepSeek distinctness on a physics-reversal event (S8).} Results 1--3 showed that on S1--S4 the DeepSeek and keyword sources are statistically indistinguishable, and on S5 (negative-price) and S7 (export-curtailed local absorption, omitted for brevity) the physical signal alone is strong enough to drive the correct action, washing out the semantic marginal contribution. A reviewer-relevant question is whether there exists any event where the LLM's semantic understanding is \emph{necessary}---where the correct response cannot be learned from the physical signal alone. We construct such an event, S8 (frequency-reserve request), where (i) the correct response (keep SOC mid-band for bidirectional reserve) is \emph{opposite} to what the physical signal suggests (PV surplus $\to$ charge $\to$ fill up), (ii) reserve-request intervals are scattered across all hours so the hour-of-day feature alone cannot predict them, and (iii) a reserve-capacity penalty $\rho(\mathrm{SOC}-0.5)^2$ is applied \emph{only} during intervals whose event text indicates a reserve request, making ``keep mid-band'' reward-contingent on the textual event. Under the main configuration, LE-DRL-SAC with DeepSeek semantic features outperforms the keyword encoder by $+788.6$ yuan (95\% CI $[+743.3, +837.5]$, 5/5 seeds favorable), and pays a lower reserve penalty ($1{,}803.7$ vs.\ $2{,}337.5$ yuan). The keyword encoder returns all-zero scores for the reserve event (its keyword table has no ``frequency/reserve'' branch), so it cannot steer the actor away from filling up during reserve intervals; DeepSeek recognizes the reserve request and returns renewable-curtailment $0.20$ and storage-bias $0.0$ (no charge bias, keep mid-band). SAC-Numeric, which has no event-text channel at all, pays the highest reserve penalty ($3{,}746.4$) and keeps mid-band only $2.5\%$ of reserve steps. This is the scenario where the language-enhanced mechanism named in the title becomes load-bearing: when the correct response reverses the physical signal and the event timing is unpredictable from numeric features alone, the LLM's semantic understanding yields a statistically significant advantage that the keyword encoder cannot match.
...
V1 (noon, summer) & held-out & -168,891 & 0.48 \\
V2 (deep, noon) & held-out & -159,917 & 0.45 \\
V3 (noon, winter) & held-out & -138,451 & 0.47 \\
V4 (noon, 4 MW) & held-out & -170,117 & 0.47 \\
\bottomrule
\end{tabular}
\end{table}
