...
\begin{abstract}
Virtual power plants (VPPs) must increasingly respond to external events---weather warnings, demand-response notices, and renewable-curtailment alerts---that arrive as natural-language text before their effects surface in numerical time series. Most reinforcement-learning (RL) dispatch controllers consume only numerical states and cannot exploit these signals. This paper proposes a language-enhanced Soft Actor-Critic framework (LE-DRL-SAC) for event-aware VPP battery dispatch in which a large language model (DeepSeek) serves \emph{only} as a semantic encoder: it converts operational text into structured risk features that are fused with numerical states and consumed by an SAC policy, and it never emits a dispatch action. The learned action is blended with a deterministic semantic safety layer whose weight is treated as event-coverage dependent, so that a hand-crafted prior governs known events while the learned actor takes over on events outside the prior's coverage. On a public-data-calibrated 15-minute dataset with four event scenarios, LE-DRL-SAC with the safety layer ($w=0.9$) attains the best average total reward, outperforming numerical SAC, a text-ablation SAC, a numeric-safety-layer ablation, rule-based control, enhanced rolling-horizon optimization, and Linear-MPC. A semantic-source ablation shows that the actor genuinely reads the LLM-derived features rather than benefiting from a wider input vector: native semantics beat zeroed semantics by $+792$ yuan (20/20 seed-scenario pairs favorable), and corrupting the scores is worse than removing them. On a frequency-reserve event whose category lies outside the deployed keyword table, the LLM encoder supplies a usable semantic vector with zero reconfiguration and exceeds the keyword-encoder baseline by $+789$ yuan (95\% CI $[+743, +838]$), delineating where an LLM encoder becomes worthwhile. Finally, a three-week validation on real German DE-LU day-ahead prices and Open-Meteo weather corroborates the adaptation under genuine, heterogeneous negative prices ($+6{,}035$ EUR, 95\% CI $[+5{,}415, +6{,}656]$, 5/5 seeds).
\end{abstract}
...
\PARstart{V}{irtual} power plants (VPPs) aggregate distributed photovoltaics (PV), battery energy storage, flexible loads, electric vehicles, and other distributed energy resources into controllable portfolios that participate in electricity markets and demand response. As renewable penetration and market-oriented operation intensify, VPP dispatch can no longer be governed solely by load, PV output, price, and state of charge (SOC): operators must also react to external information such as weather warnings, market announcements, demand-response invitations, and renewable-curtailment notices. Critically, these signals often arrive as natural-language text \emph{before} their effects become visible in the numerical time series, so a controller that reads only numerical states responds to such events only after the fact.

Deep reinforcement learning (DRL) has been widely investigated for energy management and power-system control, owing to its ability to learn sequential policies directly from interaction data \cite{b1,b2,b9}. For battery dispatch, continuous-action algorithms such as Soft Actor-Critic (SAC) are especially attractive because they emit charging and discharging power commands directly \cite{b3,b11,b12}. Most DRL dispatch controllers, however, consume only numerical states, and this neglect of operational text limits their event awareness---most visibly during high-temperature load-pressure, price-spike, and renewable-curtailment intervals, when the decisive information first appears in text.

Large language models (LLMs) offer a natural interface between unstructured operational text and numerical decision models, and recent studies have explored them for power-system reasoning, dispatch assistance, and broader energy-sector applications \cite{b4,b5,b6}. Commissioning an LLM to emit the final dispatch action directly, however, is risky in power-system operation, where every command must satisfy battery limits, SOC dynamics, market-settlement logic, and reproducibility requirements. A more tractable design uses the LLM as a semantic front end: it transforms event text into structured risk features, while a conventional controller or an RL policy produces the final action.

This study develops such a decoupled design for VPP battery dispatch. The language module emits risk scores rather than free-form dispatch commands; the SAC actor provides the learned control component; and a deterministic semantic safety layer maps high-confidence textual risk cues into bounded action corrections. The design is motivated by a practical observation: a purely learned actor tends to underuse rare event signals, whereas a pre-designed rule or prior is reliable only within the event categories it explicitly covers. When a new event category falls outside that coverage, cached LLM-derived semantic features can make the event observable to the learning actor---without letting the language model bypass physical feasibility checks. Related work is reviewed in Section~\ref{sec:related}.
...
Virtual power plants and battery storage dispatch have traditionally been formulated as mixed-integer linear programming (MILP), stochastic programming, or robust optimization problems that explicitly model forecast uncertainty, battery dynamics, and market settlement \cite{b10,b13,b14,b15}. Stochastic-programming offering models for VPP participation in electricity markets are reviewed in \cite{b10}, while model predictive control (MPC) has become a standard operational baseline for building and microgrid energy management \cite{b7,b16}. Although these optimization methods are interpretable and constraint-aware, they depend heavily on accurate forecasts and careful parameter tuning, and their online solving cost grows with the horizon and the state dimension. This paper retains a rolling-horizon optimizer family as the optimization baseline, including an engineering-regularized enhanced variant, and additionally implements a Linear-MPC reference (Scipy HiGHS LP solver, 24-step rolling horizon, terminal SOC penalty) that solves an explicit forecast-constrained LP at each step, anchoring the comparison against a receding-horizon optimizer.
...
The case study uses a 15-minute VPP dispatch dataset calibrated against public Chinese power-sector disclosures. Project metadata identify the load, PV, temperature, and event trajectories as generated sequences rather than private measured telemetry; accordingly, the data should be read as public-data-calibrated dispatch sequences (hereafter, the calibrated VPP dataset), not as complete Guangdong spot-market clearing records or verified operating logs. Table~\ref{tab:data_boundary} summarizes the data boundary: public disclosures supply the macro electricity-demand, renewable, and market-price background, while the 15-minute dispatch trajectories and operational event text are generated by a reproducible scenario generator from realistic event categories and expression patterns (weather warnings, market-price alerts, demand-response notices, and renewable-curtailment notices).
...
The VPP comprises aggregate load, PV generation, and one battery storage system. At each 15-minute step the policy observes the current state and outputs a continuous battery action, with positive action denoting discharge and negative action denoting charge; infeasible actions are clipped by the SOC and power constraints.
...
- c_{\mathrm{vio}} I_t ,
\end{equation}
where $q^{\mathrm{net}}_t$ is net export after the battery action, $c_{\mathrm{deg}}$ is the degradation cost, $q^{\mathrm{cur}}_t$ is curtailed renewable energy, and $I_t$ indicates action infeasibility. Training uses an advantage-style reward relative to the no-action baseline so that the actor focuses on the marginal value of battery control, while testing reports the original VPP operating reward. Because the reward is a monetary quantity derived from the price series, it is denominated in the currency of the underlying price data: yuan for the calibrated S1--S5 scenarios and EUR for the real-price DE-LU validation (Section~\ref{sec:unseen_event}). No exchange-rate conversion is applied; the reported figures should be read as relative advantages over the no-action baseline rather than as cross-currency-comparable absolute revenues.

\section{Proposed LE-DRL-SAC Framework}
\label{sec:method}

\subsection{Architecture}
Fig.~\ref{fig:architecture} shows the complete framework; this subsection introduces its base components (LLM semantic encoder, SAC actor, and semantic safety layer), and the learned event-coverage gate that automates the safety-layer weight is developed in Section~\ref{sec:gated_moe}. A large language model (DeepSeek) serves as the semantic encoder, with a deterministic local keyword-based encoder reserved as a fallback when the API is unavailable. The LLM is queried once per distinct event template and the resulting scores are cached, so it does not participate in the control loop at inference time and incurs no per-step latency. The semantic output is the compact vector
\begin{equation}
s^{\mathrm{sem}}_t =
...
\end{equation}
where $w \in [0,1]$ is the safety-layer blending weight, and the final action is then clipped by the battery power and SOC feasibility limits. The layer does not let the language model emit arbitrary dispatch commands; it only maps structured risk scores into a bounded charging/discharging correction. The weight $w$ is treated as event-coverage-dependent rather than a universal constant: on the event categories the prior was designed for, the prior is close to scenario-optimal and a high $w$ is preferred, whereas on categories absent from the prior design the prior has no matching branch and a low $w$ lets the learned actor take over. In the main experiment on the four known-event scenarios, $w=0.9$ is selected from a prior-weight sweep because it outperforms every implemented baseline, including the rule-based controller, while retaining a nonzero learned SAC component; the semantic prior amplitude is bounded by a guidance power of $2.0$~MW. Section~\ref{sec:unseen_event} examines the complementary regime---a new negative-price event category introduced during adaptation---where $w$ is set to zero and the semantic-consistency regularizer is relaxed so that the learned actor, advised by the LLM-derived features, produces the final action. The two configurations instantiate the event-coverage principle, and the transition between them is later automated by a learned gate (Section~\ref{sec:gated_moe}). To isolate the contribution of textual semantics from the safety-layer mechanism itself, a diagnostic variant replaces the semantic prior with a purely numeric prior $a^{\mathrm{num}}_t$ derived from price, SOC, hour, and PV surplus only; the same blending formula yields a numeric safety layer used solely as an ablation baseline, not as the final controller.
...
\begin{itemize}
  ...
  \item \textbf{Rolling-Horizon}: solver-free rolling-horizon dynamic-programming optimizer with a 16-step horizon, a discrete action grid, and a SOC grid; a strong operational benchmark with short-horizon look-ahead.
  \item \textbf{Enhanced Rolling-Horizon}: an engineering-regularized MPC-like rolling optimizer that augments the rolling-horizon baseline with a terminal SOC value, an action-smoothing penalty, and an additional cycling penalty; it exposes the throughput--reward trade-off of aggressive short-horizon optimization.
  \item \textbf{Linear-MPC}: a receding-horizon LP baseline (Scipy HiGHS, 24-step horizon, terminal SOC penalty) that solves a deterministic forecast-constrained LP at each step under explicit SOC, power, and curtailment constraints.
  \item \textbf{SAC-Numeric + numeric safety layer}: a diagnostic variant of SAC-Numeric whose learned action is blended with a purely numeric safety prior $a^{\mathrm{num}}_t$ via the same formula as the semantic safety layer but without textual information, isolating the safety-layer mechanism from language-derived semantics.
...
SAC-Numeric, LE-DRL w/o Text, and LE-DRL-SAC are trained for 80 episodes on S1--S4 with three seeds (2026, 2031, 2042). LE-DRL-SAC uses LLM-derived semantic features with semantic auxiliary reward scale 0.35 and actor semantic-consistency weight $\lambda_{\mathrm{sem}}=3.0$. The safety-layer weight is then swept over $w \in \{0,0.25,0.50,0.75,0.85,0.90,1.00\}$ at a fixed guidance power of $2.0$~MW; the endpoints $w=0$ and $w=1$ correspond to the pure learned actor and the pure semantic prior, respectively. The new-event adaptation and parameter-variant studies (Sections~\ref{sec:unseen_event}--\ref{sec:gated_moe}) extend the training set to S1--S5 and use five seeds (2026, 2031, 2042, 2047, 2053) to strengthen the statistical evidence on the negative-price event category and its held-out variants.
...
Table~\ref{tab:weight_sweep} reports the safety-layer weight sweep. Increasing $w$ improves total reward on S1--S4, because the semantic prior supplies direct event-responsive corrections for the event categories it was designed to cover. The main configuration uses $w=0.9$, which outperforms every implemented baseline, including the rule-based controller, while retaining a nonzero learned SAC component. We note that $w=1.0$ yields a marginally higher reward (-208,278.4 vs.\ -208,468.9 yuan), but at $w=1.0$ the controller degenerates to the prior alone and the SAC actor is unused at test time. We therefore report $w=1.0$ as an in-sample prior upper bound and retain $w=0.9$ as the main hybrid configuration: the prior is a scenario-specific mapping designed with full knowledge of the four categories and cannot adapt without being rewritten, whereas in deployment event categories and market conditions evolve and the learned SAC component supplies the adaptive capacity examined in the new-event adaptation and gate studies. The contribution is thus a coverage-dependent hybrid-control principle, not a claim that the SAC actor alone dominates the known-event prior.
...
\midrule
0.00 & -210,447.8 & -697.1 & 14.35 \\
0.25 & -209,874.6 & \textbf{-691.4} & 16.82 \\
...
The headline total-reward gap over the rule-based baseline is small in percentage terms (about 0.24\%), because most of the total reward comes from the net-export revenue that any feasible policy accrues, so the marginal value of battery control is small relative to the absolute reward. Three observations show that the improvement is nonetheless substantive. First, on tail risk the proposed controller improves CVaR 5\% from -705.0 yuan (Rule-Based) to -696.7 yuan, a 1.2\% reduction in worst-step losses, which is more operationally meaningful than the headline figure. Second, it raises the high-price discharge rate from 0.176 to 0.231, a 31\% relative increase, showing that the policy concentrates discharging in high-price intervals rather than merely increasing throughput. Third, the advantage is scenario-dependent and largest where textual semantics should matter most: in the price-spike scenario the proposed controller reaches -218,660.5 yuan versus -219,146.3 yuan for Rule-Based, and the out-of-distribution evaluation confirms that the event-driven scenarios (S2, S4) are where the language-enhanced policy consistently leads. We therefore report the total-reward gap alongside CVaR and behavioral metrics rather than relying on the headline percentage alone.
...
\textbf{Interpretation.} Two conclusions follow. First, LLM-derived semantic features, consumed as state by a learning actor that is free to learn from them, let the policy adapt after a new event category is introduced and the hand-crafted prior cannot cover it. The result is evidence for semantic-feature-assisted SAC adaptation; it is not a claim that an LLM policy is superior to conventional control rules, since the final action remains learned and feasibility-constrained. Second, the result sharpens the event-coverage principle that motivates the safety-layer design: on known events the prior dominates and a strong regularizer is harmless, but on newly introduced events the learned actor must take over and a strong regularizer is harmful. This is why the safety-layer weight is treated as event-coverage-dependent in this paper: a single fixed weight cannot be optimal across both regimes. The present paper establishes the principle with two fixed configurations (high weight on known events, zero weight with relaxed regularizer during new-event adaptation) and reports the trade-off explicitly; automating the coverage-dependent weight selection is examined in Section~\ref{sec:gated_moe}.
...
This paper has presented LE-DRL-SAC with a semantic safety layer for event-aware VPP battery dispatch. The framework uses an LLM-compatible semantic encoder to turn operational text into structured risk features, fuses them with numerical states, trains an SAC actor under semantic-consistency guidance, and blends the learned action with a bounded semantic prior at test time. On the calibrated VPP dataset, the final controller ($w=0.9$) attains the best average total reward across four event scenarios, outperforming numerical SAC, a text-ablation SAC, a numeric-safety-layer SAC variant, Rule-Based control, a Linear-MPC optimizer, the Rolling-Horizon optimizer, and the engineering-regularized Enhanced Rolling-Horizon optimizer, with seed-paired bootstrap intervals over three seeds for the main comparison and five seeds for the adaptation studies. Two ablations sharpen the claim: a numeric safety layer improves SAC-Numeric yet stays below the language-enhanced controller, so the gain is not solely from the blending mechanism; and a semantic-source ablation shows that the actor genuinely reads the semantic state (native semantics beat zeroed semantics by $+792$ yuan, 20/20 pairs; corrupting the scores is worse than removing them), that a keyword encoder matches DeepSeek on the four known categories it was built for, and that the LLM's distinct value appears on a frequency-reserve event outside that table, where it generalizes with zero reconfiguration. Generalization is supported by an out-of-distribution check on real Guangzhou weather (July 2024), which retains the highest average reward under a weather-driven distribution shift, and by an unseen-expression test, which confirms the policy is driven by semantic content rather than memorized templates.

The central methodological finding is that the relative value of the learned actor and the semantic prior is governed by event coverage. On the four training event categories, the prior---designed with full knowledge of those events---dominates and the learned actor contributes a bounded correction; when a negative-price-surplus event absent from the prior is introduced during adaptation, the learned actor must take over, and the LLM-derived features let it adapt to a degree that a text-agnostic SAC cannot match ($+5{,}482$ yuan, 95\% bootstrap CI $[+4{,}875, +6{,}074]$, all five seeds favorable). This finding refines the design of language-enhanced reinforcement-learning controllers by separating two roles of semantic information: a state feature that makes a newly described event learnable by the SAC actor; and a training target that constrains the actor, which helps on known events but suppresses adaptivity on newly introduced events (a regularizer sweep shows an 18{,}515-yuan gain from relaxing the constraint, with no degradation on known events). The claim is correspondingly narrow: LLM-derived event semantics help a conventional SAC policy adapt when the fixed rule prior lacks event coverage, rather than an LLM directly outperforming a rule controller. A parameter-variant study across four structurally distinct negative-price scenarios confirms that the adaptation is robust across negative-price depth, season, and PV plant capacity. Most importantly, a validation built entirely from real German DE-LU day-ahead prices (a three-week series with 14 negative-price days and a minimum of $-99.0$~EUR/MWh) and real Open-Meteo weather---the evidence closest to a real market in this paper---confirms that the adaptation transfers to genuine, heterogeneous negative-price records rather than to the synthetic generator alone (gap $+6{,}035$ EUR, 95\% CI $[+5{,}415, +6{,}656]$, 5/5 seeds favorable), grounding the event-awareness claim of the title in real data. Finally, a learned event-coverage gate---a small network trained on per-batch-normalized $Q$-differences between a regularized and a free SAC expert---recovers the high-$w$-on-known / low-$w$-on-negative-price split from data alone, without coverage labels, demonstrating that the event-coverage weight can be automated. Future work will optimize the gate's blended-action reward, validate the framework on measured VPP event logs, incorporate full MILP/MPC baselines, and introduce formal safety-constrained RL to enhance deployability.
