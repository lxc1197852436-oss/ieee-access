...
\begin{enumerate}
  ...
  \item An unseen-event adaptation study is reported. A fifth event category—negative-price deep surplus—that was not used during prior design or actor training is constructed, and the LLM semantic encoder is queried once for this unseen event to obtain cached risk scores. The study shows that when the semantic regularizer is relaxed, the language-enhanced policy exceeds numerical SAC on this unseen event (mean reward gap +5,482 yuan, 95\% bootstrap CI [+4,875, +6,074], all five seeds favorable), providing evidence that LLM-derived features help a reinforcement-learning dispatch policy adapt to an event category that the hand-crafted prior cannot cover. Crucially, this adaptation is not confined to the calibrated data: it is corroborated on a validation scenario built entirely from \emph{real} German DE-LU day-ahead prices and \emph{real} Open-Meteo weather observations, where the language-enhanced policy retains a statistically significant advantage under genuine, much shallower negative prices (gap $+2{,}201$ EUR, 95\% CI $[+593, +3{,}627]$, four of five seeds favorable). This real-price, real-weather result is the evidence closest to deployment and is the empirical anchor for the event-awareness claim in the title.
...
Recent surveys examine LLM capabilities and limitations in the electric energy sector. Majumder \emph{et al.}~\cite{b4} map the landscape of LLM use cases in electricity generation, transmission, and consumption, identifying knowledge retrieval, dispatch assistance, and text-driven decision support as the most promising directions while documenting hallucination and auditability risks that motivate keeping the LLM out of the final control loop. Amjad \emph{et al.}~\cite{b6} provide a complementary 2025 review of LLM applications across electrical power and energy systems, cataloguing architectural patterns from prompt-based reasoning to retrieval-augmented agents, and noting that operational deployment is constrained by reproducibility and safety requirements. Beyond surveys, Cheng \emph{et al.}~\cite{b5} propose an LLM for advanced power dispatch that uses language reasoning over operational contexts to support dispatch decisions, illustrating the potential and the limitations of letting an LLM participate in the decision chain. Pre-trained encoders such as BERT and domain-adapted language models have likewise been employed to embed Chinese power-system text into downstream prediction and classification tasks \cite{b21,b22}, treating the language model as a feature extractor rather than a controller. A common pattern across this literature is to deploy the language model as a feature extractor, advisor, or explainer rather than as the final controller, because directly LLM-generated actions are difficult to audit and constrain \cite{b4,b6}. The present work adheres to this pattern: the language module emits structured risk scores only, whereas a reproducible SAC actor and a deterministic safety layer generate the final action, so the LLM is queried once per event template and never participates in the per-step control loop. We note that the existing LLM-for-power-systems literature is surveyed here at the level of architectural patterns rather than as a systematic catalogue of every LLM+dispatch pairing; a more exhaustive review is left to dedicated survey work, and the present paper focuses on the specific question of whether LLM-derived semantics help a reinforcement-learning dispatch policy adapt to event categories that a hand-crafted prior cannot cover.
...
S5 (neg-price, in train) & neg-price & -154,189 & 0.31 \\
