...
\begin{document}
\history{This manuscript draft is prepared for IEEE Access submission development.}
\doi{}
...
\begin{abstract}
Virtual power plants (VPPs) increasingly operate under renewable uncertainty, market-price volatility, and external operational events such as weather warnings, demand-response notices, and renewable-curtailment alerts. Most reinforcement-learning-based dispatch methods use numerical states only and therefore cannot directly exploit such natural-language information. This paper develops a language-enhanced deep reinforcement learning framework for event-aware VPP battery dispatch. The proposed LE-DRL-SAC architecture uses a large-language-model-compatible or local semantic encoder to convert operational text into structured risk features, including general risk, price-spike pressure, load pressure, and renewable-curtailment pressure. These features are fused with numerical states and used by a Soft Actor-Critic (SAC) policy; the language model does not directly output dispatch actions. To improve operational reliability, the learned SAC action is further combined with a deterministic semantic safety layer that encodes risk-consistent charging and discharging preferences. The framework is evaluated on a public-data-calibrated 15-minute VPP dispatch dataset containing load, photovoltaic generation, electricity price, temperature, battery state of charge, and Chinese operational event text. Four scenarios are tested: normal summer operation, high-temperature load pressure, price spike, and renewable-curtailment pressure. Across three random seeds, the proposed LE-DRL-SAC with semantic safety layer ($w=0.75$) achieves an average total reward of -208,764.0 yuan, outperforming a rule-based controller (-208,969.9 yuan), a rolling-horizon optimizer (-210,616.7 yuan), numerical SAC (-211,023.2 yuan), and a text-ablation SAC (-211,023.1 yuan). Text ablation and prior-weight sweeps confirm that the improvement comes from operational text semantics and the safety-layer design rather than input-dimensionality expansion alone. The results suggest that textual risk semantics can improve event-driven VPP dispatch when they are coupled with a reproducible action-safety mechanism, while validation on measured VPP telemetry and full MILP/MPC baselines remains necessary.
\end{abstract}
...
\midrule
\multirow{6}{*}{S1} & LE-DRL-SAC + safety layer & \textbf{-201,809.8} & -638.0 & 22.99 & 0.256 \\
...
& LE-DRL-SAC, actor only & -202,976.7 & \textbf{-628.4} & 13.84 & \textbf{0.528} \\
\midrule
\multirow{6}{*}{S2} & LE-DRL-SAC + safety layer & \textbf{-218,935.7} & -713.4 & 20.98 & 0.218 \\
...
& LE-DRL-SAC, actor only & -219,794.2 & \textbf{-704.7} & 12.76 & \textbf{0.534} \\
\midrule
\multirow{6}{*}{S3} & LE-DRL-SAC + safety layer & -218,894.6 & -792.5 & 23.61 & 0.276 \\
...
\midrule
\multirow{6}{*}{S4} & LE-DRL-SAC + safety layer & \textbf{-195,416.0} & \textbf{-616.2} & 34.04 & 0.339 \\
...
Table~\ref{tab:weight_sweep} reports the semantic safety-layer weight sweep. Increasing $w$ improves total reward because the semantic prior provides direct event-responsive charging and discharging corrections. However, $w=1.0$ removes the learned actor contribution and effectively becomes a semantic-prior controller. The main paper therefore uses $w=0.75$, which outperforms the strong baselines while retaining a nonzero learned SAC component.
