...
Deep reinforcement learning (DRL) has been widely explored for energy management and power-system control because it can learn sequential policies from interaction data \cite{b1,b2}. In battery dispatch, continuous-action algorithms such as Soft Actor-Critic (SAC) are attractive because they can directly output charging and discharging power \cite{b3}. However, most DRL dispatch models use only numerical states. Ignoring operational text may limit event awareness, especially in high-temperature load-pressure periods, price-spike periods, and renewable-curtailment periods.

Large language models (LLMs) provide a possible interface between unstructured operational text and numerical decision models. Recent studies have explored LLMs for power-system reasoning, dispatch assistance, and energy-sector applications \cite{b4,b5,b6}. Directly asking an LLM to produce final dispatch actions is risky because VPP operation must satisfy battery limits, SOC dynamics, market settlement logic, and reproducibility requirements. A more practical role is to use the LLM as a semantic front end: it converts event text into structured risk features, while a conventional controller or reinforcement-learning policy produces the final action.
...
\begin{enumerate}
  ...
  \item The method is compared with numerical SAC, a text-ablation SAC, a deterministic rule-based controller, a solver-free rolling-horizon optimizer, random control, and Soft-Q baselines. The final configuration outperforms the implemented rule-based and rolling-horizon baselines in average total reward.
\end{enumerate}
